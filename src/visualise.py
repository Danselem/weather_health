...
class EDAReport:
    ...
    def __init__(self, data: pd.DataFrame, target_col: str = None, output_prefix: str = 'eda_report'):
        self.data = data
    ...
    def profile_report(self):
        ...
        logger.info(f"EDA complete. Files saved in '{self.output_dir}':\n- {self.html_path.name}")
    ...
    def plot_missing_values(self, pdf):
        ...
        if not missing_nonzero.empty:
            miss_df = pd.DataFrame((missing_nonzero * 100 / self.data.shape[0])).reset_index()
    ...
    def plot_value_counts(self, pdf):
        ...
        categorical_cols = [
            ...
            if (self.data[col].nunique() <= 10 and col not in numeric_cols) or self.data[col].nunique() <= 2
        ]
